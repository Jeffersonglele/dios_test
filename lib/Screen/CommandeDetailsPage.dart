...
class _CommandeDetailsPageState extends State<CommandeDetailsPage> {
  ...
  Widget _buildHeader(String status, Color statusColor) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Text('Commande #${_commande.commandeID}',
  ...
  Widget _buildStatusTimeline(String currentStatus) {
    final steps = [
      _StepData(CommandeStatus.pending, 'En attente', Icons.hourglass_top_rounded),
      _StepData(CommandeStatus.paid, 'Payée', Icons.payment_rounded),
      _StepData(CommandeStatus.confirmed, 'Confirmée', Icons.check_circle_rounded),
      _StepData('Livrée', 'Livrée', Icons.delivery_dining_rounded),
    ];
      ...
      child: Column(
        ...
        children: [
          ...
          if (_restaurantName.isNotEmpty)
            _buildInfoRow(
                Icons.store_rounded, 'Restaurant', _restaurantName),
          _buildInfoRow(Icons.person_rounded, 'Client', _clientName),
          if (_clientPhone.isNotEmpty)
            _buildInfoRow(
                Icons.phone_rounded, 'Téléphone', _clientPhone),
          _buildInfoRow(
              Icons.payment_rounded, 'Paiement', _paymentLabel),
          if (_commande.note != null && _commande.note! > 0)
            _buildInfoRow(Icons.star_rounded, 'Note',
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                Text(_dishNames[l.platID] ?? 'Plat #${l.platID}',
                    style: AppTypography.bodyMedium(
                        color: AppColors.resolve(AppColors.ink, AppDarkColors.ink))),
                Text('${l.prixUnitaire.toStringAsFixed(2)} € · x${l.quantite}',
                    style: AppTypography.bodyMedium(
                            color: AppColors.resolve(AppColors.inkMuted, AppDarkColors.inkMuted))
          ...
          Text('${(l.prixUnitaire * l.quantite).toStringAsFixed(2)} €',
  ...
  Widget _buildTotals() {
    return Container(
      ...
      child: Column(
        children: [
          _buildTotalRow('Sous-total', '${_subtotal.toStringAsFixed(2)} €'),
          const SizedBox(height: 6),
          _buildTotalRow('Livraison',
              '${_commande.fraisLivraison.toStringAsFixed(2)} €'),
          if (_commande.reduction > 0) ...[
            const SizedBox(height: 6),
            _buildTotalRow('Réduction',
                '-${_commande.reduction.toStringAsFixed(2)} €'),
          ],
          Divider(height: 24,
              color: AppColors.resolve(AppColors.border, AppDarkColors.border)),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text('Marquer comme assigné'),
                style: ElevatedButton.styleFrom(
                  backgroundColor:
