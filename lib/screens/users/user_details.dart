                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                Text(_statusLabel(l10n), style: AppTypography.labelMedium(color: _statusColor())),
                              ],
                            ),
                          ),
                          const Spacer(),
                      ...
                      if (_isEditing) ...[
                        _buildSectionCard(
                          ...
                          child: Column(children: [
                            _buildFormField(controller: _firstnameCtrl, label: l10n.user_details_firstname, validator: (v) => (v?.trim().isEmpty ?? true) ? l10n.user_details_required : null),
                            const SizedBox(height: 12),
                            _buildFormField(controller: _lastnameCtrl, label: l10n.user_details_lastname, validator: (v) => (v?.trim().isEmpty ?? true) ? l10n.user_details_required : null),
                            const SizedBox(height: 12),
                            _buildFormField(controller: _emailCtrl, label: l10n.user_details_email, keyboardType: TextInputType.emailAddress, validator: (v) => (v?.trim().isEmpty ?? true) ? l10n.user_details_required : null),
                            const SizedBox(height: 12),
                            _buildFormField(controller: _phoneCtrl, label: l10n.user_details_phone, keyboardType: TextInputType.phone),
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                child: _isSaving
                                    ...
                                    : Text(l10n.save_profile, style: const TextStyle(fontWeight: FontWeight.w600)),
                              ),
                            ),
                          ]),
                        ),
                        ...
                        _buildSectionCard(
                          ...
                          color: AppColors.brand,
                          child: Column(children: [
                            _infoTile(Icons.email_outlined, l10n.user_details_email, current_user!.email, AppColors.brand),
                            const _Divider(),
                            _infoTile(Icons.phone_android_rounded, l10n.user_details_phone, current_user!.telephone, AppColors.success),
                            const _Divider(),
                            _infoTile(Icons.public_rounded, l10n.user_details_country, current_user!.country, Colors.blue),
                            const _Divider(),
                            _infoTile(Icons.badge_rounded, l10n.user_details_profile_type, type_profil, roleColor),
                            const _Divider(),
                            _infoTile(_statusIcon(), l10n.user_details_identity, _statusLabel(l10n), _statusColor()),
                          ]),
                        ),
                      ],
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(l10n.force_delete, style: const TextStyle(color: AppColors.error, fontWeight: FontWeight.w600, fontSize: 15)),
                                      const SizedBox(height: 2),
                                      Text(l10n.force_delete_desc, style: AppTypography.bodyMedium(color: AppColors.error.withValues(alpha: 0.7)).copyWith(fontSize: 12)),
                                    ],
                                  ),
                                ),
          ...
          child: isImg
              ...
              : Column(
                  ...
                  children: [
                    ...
                    Text(l10n.pdf_document, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                    const SizedBox(height: 2),
                    Text(l10n.tap_to_open, style: AppTypography.bodyMedium(color: AppColors.inkMuted)),
                  ],
                ),
        ),
      ),
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            const SizedBox(width: 10),
            Text(l10n.validation, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
          ]),
          const SizedBox(height: 14),
          Row(children: [
            Expanded(
      ...
      builder: (ctx) {
        ...
        builder: (ctx, set) => AlertDialog(
          title: Row(children: [
            ...
            const SizedBox(width: 8),
            Flexible(child: Text(l10n.delete_permanently, style: const TextStyle(color: AppColors.error, fontWeight: FontWeight.w600))),
          ]),
          content: Column(
              ...
              onPressed: deleting ? null : () async {
                ...
                try {
                  ...
                  if (res.success) {
                    ...
                    if (!ctx.mounted) return;
                    Toast(context, l10n.user_deleted(current_user!.firstname!), true);
                    Navigator.pop(ctx);
                    Navigator.pop(context);
                  } else {
                    Toast(context, l10n.error_with_message(res.error?.toString() ?? res.result?.toString() ?? ''), false);
                  }
                } catch (e) {
  ...
  void _showRejectDialog(Users users) {
    ...
    showDialog(
      ...
      builder: (ctx) {
        ...
        title: Row(children: [const Icon(Icons.close_rounded, color: AppColors.error, size: 24), const SizedBox(width: 8), Text(l10n.reject_profile)]),
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(l10n.send, style: const TextStyle(color: AppColors.error)),
          ),
        ],
      );
  },);
  ...
  Future<void> _validateUsers(Users users) async {
    ...
    if (updateResult == "success") {
      ...
      if (emailSent) {
        ...
        Toast(context, l10n.profile_validated_email_sent(users.firstname!, users.lastname!), true);
      } else {
  ...
  Future<void> _rejectUsers(Users users, String remark) async {
    ...
    if (updateResult == "success") {
      ...
      if (emailSent) {
        ...
        Toast(context, l10n.profile_rejected_email_sent(users.firstname!, users.lastname!), false);
      } else {
